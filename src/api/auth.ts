...
export const GetProfileUserName = async (accountId: string) => {
  const obj = {
    public_address: accountId,
  };
  ...
  const profileResult = await fetch(`${API_URI}/get_or_create_username`, {
    method: "POST",
    body: jsonObj,
  });
...
export const GetProfilePicture = async (accountId: string, token: string) => {
  const obj = {
    public_address: accountId,
  };
  ...
  try {
    const profileResult = await fetch(
      `${API_URI}/get_profile_image/${accountId}`,
      requestOptions
    );
    console.log(profileResult);

    const picture = await profileResult.text();
    return picture;
  } catch (error) {
